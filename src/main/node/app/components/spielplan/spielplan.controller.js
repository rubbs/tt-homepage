...
.controller('SpielplanCtrl', ['$scope', '$filter', 'spielplan', function ($scope, $filter, spielplan) {
  $scope.spielplan = spielplan.get(function(){
    console.log('spielplan loaded', $scope.spielplan);

    for (var m in $scope.spielplan.matches){
        var match = $scope.spielplan.matches[m];
  ...
  $scope.onPdf = function () {
    ...
    pdf.setFontSize(12);
    var ordered = $filter('orderBy')($scope.spielplan.matches, 'date');
